...
{ 
	...
	struct _copy_functor
	{
		...
		OutputIterator operator() (InputIterator first1, InputIterator last1, OutputIterator first2)
		{
	...
	{
		T* operator() (T* first1, T* last1, T* first2)
		{
	...
	{
		T* operator() (const T* first1, const T* last1, T* first2)
		{
			typedef type_traits<T>::has_trivial_assignment_ctr has_trivial_assignment_ctr;
			return _copy_by_trivial_assign_ctr_4_ptr(first1, last1, first2, has_trivial_assignment_ctr());
		}
	...
	template<typename T>
	T* _copy_by_trivial_assign_ctr_4_ptr(T* first1, T* last1, T* first2, _true_type)
	{	
		size_t size = last1 - first1;
	...
	template<typename T>
	T* _copy_by_trivial_assign_ctr_4_ptr(T* first1, T* last1, T* first2, _false_type)
	{
	...
	template<typename InputIterator, typename OutputIterator>
	OutputIterator _copy_by_category(InputIterator first1, InputIterator last1, OutputIterator first2, input_interator_tag)
	{
